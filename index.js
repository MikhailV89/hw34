	...
	renderCourses() {
		if (this.courses && this.courses.length > 0) {
			const coursesList = this.courses.map((course) => {
				...
				const markInfo = this.getGradeText(courseMark);

				return this.role === "student"
					? `<div class="user__courses--course ${this.role}">${course.title}: <span class="${markInfo}">${markInfo}</span></div>`
					: this.role === "admin"
					? `<div class="admin__info ${this.role}"><b>Title: ${course.title}</b> <br> <b>Admin's score: <span class="${markInfo}">${markInfo}</span></b> <br> <b>Lector: ${course.lector}</b></div>`
					: this.role === "lector"
					? `<div class="admin__info ${this.role}"><b>Title: ${course.title}</b> <br> <b>Lector's score: <span class="${markInfo}">${markInfo}</span></b> <br> <b>Average student's score: <span class="${this.getGradeText(course.studentsScore)}">${this.getGradeText(course.studentsScore)}</span></b></div>`
					: "";
			});
...
document.write(`<div class="users">${renderUsers.join('')}</div>`);